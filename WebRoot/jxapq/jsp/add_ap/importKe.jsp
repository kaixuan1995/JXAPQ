...
<script type="text/javascript">
	...
	$(function(){
		...
		document.getElementById("sortedLevel_value").value=kingdom+"--"+phylum+"--"+clazz+"--"+order;
  ...
  <body>
    <p>
  <div id="box_top">批量导入科
 <c:if test="${userList[0].user_type==3 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
   <input type="button"  value="下载植物模板" class="ui_input_btn01" onclick='downloadTemplate("科模板(植物).xls")'/> &nbsp;
 </c:if>
 <c:if test="${userList[0].user_type==2 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
   <input type="button"  value="下载动物模板" class="ui_input_btn01" onclick='downloadTemplate("科模板(动物).xls")'/> &nbsp;
 </c:if>
  </div>
  </p>
  <div class="ui_content">
    <form id="importForm" name="myForm" enctype="multipart/form-data" method="post" action="${pageContext.request.contextPath}/UtilServlet?method=importData">
      <table  cellspacing="0" cellpadding="0" width="100%" align="left" border="0">
       ...
       <c:if test="${userList[0].user_type==3 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
   			<input type='hidden' name="voName" value="addKeViewZhiWu" />
 	   </c:if>
	<c:if test="${userList[0].user_type==2 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
		     <input type='hidden' name="voName" value="addKeViewDongWu" />
	</c:if>
      ...
      <tr>
          ...
          <td height="42" colspan="2" >
            ...
            <select name="foreign_key_value" id="order" class="ui_select01">
