...
<script type="text/javascript">
	$(document).ready(function() {	
		...
		$("#submitbutton").click(function(){
			//$('#sortedLevel_value').val($('#species_level').val());
			$('#importForm').submit(); 	
		});
		var result = 'null';
		if(result =='success'){
	...
	$(function(){
		...
		document.getElementById("sortedLevel_value").value=kingdom+"--"+phylum+"--"+clazz;
//alert(document.getElementById("sortedLevel_value").value);
  ...
  <body>
     <p>
  <div id="box_top">批量导入目
  <input type="button"  value="下载模板" class="ui_input_btn01" onclick='downloadTemplate("目模板.xls")'/> &nbsp;
  </div>
  </p>
 ...
 <form id="importForm" name="myForm" enctype="multipart/form-data" method="post" action="${pageContext.request.contextPath}/UtilServlet?method=importData">
 	<input type='hidden' name="voName" value="addMuView" />
 	<input type='hidden' name='sortedLevel_value' id="sortedLevel_value"/>
      ...
      <tr> 
		...
		<tr>
          <td height="37" class="ui_text_rt" >目文件：</td>
