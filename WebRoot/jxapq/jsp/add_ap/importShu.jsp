...
<script type="text/javascript">
	$(document).ready(function() {
		$(function(){
			...
			document.getElementById("sortedLevel_value").value=kingdom+"--"+phylum+"--"+clazz+"--"+order+"--"+family;
  ...
  <body>
     <p>
  <div id="box_top">批量导入属
<c:if test="${userList[0].user_type==3 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
  <input type="button"  value="下载植物模板" class="ui_input_btn01" onclick='downloadTemplate("属模板(植物).xls")'/> &nbsp;
</c:if>
<c:if test="${userList[0].user_type==2 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
  <input type="button"  value="下载动物模板" class="ui_input_btn01" onclick='downloadTemplate("属模板(动物).xls")'/> &nbsp;
</c:if> 
  </div>
  </p>
  <div class="ui_content">
   <form id="importForm" name="myForm" enctype="multipart/form-data" method="post" action="${pageContext.request.contextPath}/UtilServlet?method=importData">
         <c:if test="${userList[0].user_type==3 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
   			<input type='hidden' name="voName" value="addShuViewZhiWu" />
 	   </c:if>
	<c:if test="${userList[0].user_type==2 }"><!-- 3植物管理员 动植物区分！！9.16会议 -->
		     <input type='hidden' name="voName" value="addShuViewDongWu" />
	</c:if>
         ...
         <tr>
          ...
          <td height="42" colspan="2">
            ...
            <select name="order_3id =" id = "order" class="ui_select01">
			 ...
			 <select name="foreign_key_value" id = "family" class="ui_select01">
            	<option selected="selected">请选择所属科</option>
          </select>
          </td>
        </tr>
		<tr>
          <td height="37" class="ui_text_rt" >属文件：</td>
