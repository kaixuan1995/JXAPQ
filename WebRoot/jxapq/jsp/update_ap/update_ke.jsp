...
<div id="container">
  <div id="nav_links"> 当前位置：动植物数据管理&nbsp;>&nbsp;<span style="color: #1A5CC6;">修改科资料</span>
    <div id="page_close"> <a href="javascript:parent.$.fancybox.close();"></a> </div>
  </div>
  <div id="box_top">修改科资料</div>
  <div class="ui_content">
    <form id="submitForm" name="submitForm" onsubmit="return getConf()" action="${pageContext.request.contextPath }/UpdateServlet?method=update&view=updateKeView" method="post">
      <table  cellspacing="0" cellpadding="0" width="100%" align="left" border="0">
        <tr>
          <td class="ui_text_rt">学名：</td>
          <td class="ui_text_lt"><input type="text" id="sname" name="#s#family_sname =" value="${list.get(0).family_sname }" class="ui_input_txt04"/>
          <input type="text" hidden="" name="#w#family_id =" value="${list.get(0).family_id }" class="ui_input_txt04"/>
          <label id="snameError"></label>
          </td>
        </tr>
        <tr>
          <td class="ui_text_rt" width="80">中文名：</td>
          <td class="ui_text_lt"><input type="text" id="cname" name="#s#family_cname =" value="${list.get(0).family_cname }" class="ui_input_txt04"/>
          <label id="cnameError"></label>
          </td>
        </tr>
		<c:if test="${userList[0].user_type==3 }"> <!-- 3代表植物 -->    
		    <tr>
		      <td class="ui_text_rt" width="80">异名（中）:</td>
		      <td class="ui_text_lt"><input type="text" name="#s#family_cnamee =" value="${list.get(0).family_cnamee }"class="ui_input_txt04"/>
		   	  </td>
		    </tr>
		</c:if>
		<c:if test="${userList[0].user_type==3 }"> <!-- 3代表植物 -->    
		    <tr>
		      <td class="ui_text_rt" width="80">异名（英）:</td>
		      <td class="ui_text_lt"><input type="text" name="#s#family_enamee =" value="${list.get(0).family_enamee }"class="ui_input_txt04"/>
		      </td>
		    </tr>
		</c:if>  
        <tr>
          <td class="ui_text_rt">英文名：</td>
          <td class="ui_text_lt"><input type="text"id="ename" name="#s#family_ename =" value="${list.get(0).family_ename }" class="ui_input_txt04"/>
           <label id="enameError"></label>
          </td>
        </tr>
        <tr>
          <td class="ui_text_rt">中文拼音：</td>
          <td class="ui_text_lt"><input type="text" id="spelling" name="#s#family_spelling =" value="${list.get(0).family_spelling }" class="ui_input_txt04"/>
        ...
        <tr>
          ...
          <td class="ui_text_lt"><textarea name="#s#family_summarize =" class="ui_tb_h30" style="width: 483px; height: 153px;">${list.get(0).family_summarize }</textarea>
          </td>
        </tr>
        <tr>
          <td class="ui_text_rt">参考文献：</td>
          <td class="ui_text_lt"><textarea name="#s#family_literature =" class="ui_tb_h30" style="width: 483px; height: 153px;">${list.get(0).family_literature }</textarea>
          </td>
        </tr>
        <tr>
          <td>&nbsp;</td>
