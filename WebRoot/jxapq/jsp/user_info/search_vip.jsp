...
<body>
	...
	/UserServlet?view=queryVipView&method=queryVip"
			method="post">
			<div>
			...
			<div class="ui_content">
				...
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_center">
							用户状态:&nbsp;<select name="user_type =" id="type">
								<option value="0">待审核vip</option>
								<option value="1">已审核vip</option>
		...
		<div class="ui_content">
			<div class="ui_tb">
				<table class="table" cellspacing="0" cellpadding="0" width="100%"
					...
					<c:forEach items="${vipList}" var="user">
						<tr>
							...
							<td>${user.user_reason }</td>
							<td><c:choose>
									<c:when test="${user.user_type == 0}">待审核vip用户</c:when>
									<c:otherwise>vip用户</c:otherwise>
								</c:choose></td>
							<td><c:choose>
									<c:when test="${user.user_type == 0}">
										<a
											href="${pageContext.request.contextPath}/UserServlet?view=agreeVipView&method=agreeVip&%23w%23user_id %3D=${user.user_id }&%23s%23user_type %3D=1">通过</a>
									</c:when>
									<c:otherwise>
										<a
											href="${pageContext.request.contextPath}/UserServlet?view=agreeVipView&method=agreeVip&%23w%23user_id %3D=${user.user_id }&%23s%23user_type %3D=0">取消vip资格</a>
									</c:otherwise>
								</c:choose></td>
						</tr>
					</c:forEach>
